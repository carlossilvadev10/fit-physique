"use client";

import React from "react";
import { Swiper, SwiperSlide } from "swiper/react";
import "swiper/css";
import "swiper/css/navigation";
import CustomButton from "./CustomButton";
import SwiperNavButtons from "./SwiperNavButtons";
import { motion } from "framer-motion";
import { fadeIn } from "@/lib/variants";

const slides = [
    {
        title: "Trabajo duro, ",
        titleAccent: "éxito seguro",
        description: "En Fit Physique, creemos en la disciplina, la pasión y la constancia como el camino para alcanzar grandes resultados."
    },
    {
        title: "Tu cuerpo, ",
        titleAccent: "tu mejor inversión",
        description: "Cada entrenamiento en Fit Physique es un paso hacia una versión más fuerte, sana y llena de energía de ti mismo."
    },
    {
        title: "Juntos, ",
        titleAccent: "hacia tus metas",
        description: "En Fit Physique no entrenas solo, entrenas con una comunidad que te impulsa, te motiva y celebra cada uno de tus logros."
    }
];

const SlideContent = ({ titleAccent, title, description }: { titleAccent: string, title: string, description: string }) => (
    <div className = "h-full flex items-center justify-center lg:justify-end pt-0 lg:py-48">
        <div className = "flex flex-col items-center lg:items-start lg:max-w-[700px]">
            <motion.h1 variants = {fadeIn("up", 0.4)} initial = "hidden" whileInView = "show" viewport = {{ once: true, amount: 0.2 }} className = "h1 text-center lg:text-left mb-2" >
                <span className = "text-accent">{title}</span>{titleAccent}
            </motion.h1>
            <motion.p variants = {fadeIn("up", 0.6)} initial = "hidden" whileInView = "show" viewport = {{ once: true, amount: 0.2 }} className = "text-white italic text-center lg:text-left mb-4" >
                {description}
            </motion.p>
            <motion.div variants = {fadeIn("up", 0.8)} initial = "hidden" whileInView = "show" viewport = {{ once: true, amount: 0.2 }} >
                <CustomButton text = "Empieza hoy" containerStyles = "w-[196px] h-[62px]" />
            </motion.div>
        </div>
    </div>
);

const HeroSlider = () => {
    return (
        <Swiper className = "h-full">
            {
                slides.map((slide, index) => (
                    <SwiperSlide key = {index}>
                        <SlideContent titleAccent = {slide.titleAccent} title = {slide.title} description = {slide.description} />
                    </SwiperSlide>
                ))
            }
            <SwiperNavButtons containerStyles = "flex justify-center lg:justify-start gap-1 absolute bottom-4 mb-6 left-1/2 -translate-x-1/2 lg:left-auto lg:right-0 lg:translate-x-0 w-auto lg:w-[700px] h-auto lg:h-[130px] z-50" btnStyles = "flex items-center justify-center w-[44px] h-[44px] lg:w-[56px] lg:h-[56px] border border-accent text-white hover:bg-accent transition-all duration-300" iconStyles = "text-xl lg:text-2xl" />
        </Swiper>
    )
}

export default HeroSlider;